/**
 * Normalize date search queries so "August", "aug", and "08" hit the same records.
 */
...
/** Pull year/month/day out of a free-text date query when possible. */
export function parseDateQuery(raw: string): ParsedDateQuery | null {
  ...
  m = t.match(
    new RegExp(`\\b(\\d{1,2})(?:st|nd|rd|th)?\\s+(?:of\\s+)?(${monthToken})(?:\\s+(\\d{4}))?\\b`, "i"),
  );
  if (m?.[1] && m[2]) {
    return {
      day: Number(m[1]),
...
/**
 * Expand a date query into lowercase substrings that should match indexed
 * birthDate / label / search_text forms (month names, 08, --08-15, etc.).
 */
export function expandDateSearchForms(raw: string): string[] {
  ...
  if (!parsed?.month) {
    // Still try bare month-number tokens inside the string
    return [...forms];
  }
...
/** True when haystack contains any expanded form of the date query. */
export function matchesDateHaystack(haystack: string, query: string): boolean {
  ...
  // Month-only: require a month signal (name or zero-padded number in a date-like place)
  ...
  // Day+month: require both a month form and a day form to reduce false positives
  if (parsed?.month != null && parsed.day != null) {
    const m = pad(parsed.month);
    const d = pad(parsed.day);
    const name = MONTH_NAMES[parsed.month - 1]!;
    const hasMonth =
      hay.includes(name) || hay.includes(`-${m}-`) || hay.includes(`--${m}`) || hay.includes(`/${m}`);
    const hasDay =
