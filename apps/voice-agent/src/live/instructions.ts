...
export function buildLiveVoiceInstructions(brain: AlfredBrain): string {
  const parts: string[] = [
    ...
    "Answer greetings, small talk, and simple clarifications yourself. Do not invent weather, weather alerts, earthquakes, space weather, wildfires, exchange rates, Hacker News stories, news headlines, crypto or metals prices, memory facts, light state, or briefing content — delegate those.",
    ...
    "When the user asks for Bitcoin or other crypto prices, delegate get_crypto_price. For gold or silver, delegate get_metals_price.",
...
export async function buildLiveBackendInstructions(brain: AlfredBrain): Promise<string> {
  ...
  return [
    ...
    "For news headlines, call get_news_headlines. For follow-ups like 'the second one' or 'tell me more about that story', call summarize_news_article with index or match.",
